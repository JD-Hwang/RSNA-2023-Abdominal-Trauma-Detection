...
class TraumaDatasetRaw(Dataset):
    def __init__(self, data_root=Path('./data/train_images/'), mode="train", xy_size=512, depth=200):
    ...
    def get_target_thickness_dicoms(self, dicom_dir, target_thick=5):
        patient_id = dicom_dir.parent.name
        series_id = dicom_dir.name
        src_dicoms = sorted(dicom_dir.glob('*.dcm'), key=lambda x: int(x.name[:-4]), reverse=True)
        n_d = len(src_dicoms)
        curr_thick = self.df_dicom.loc[(patient_id, series_id), "SliceThickness"].mean()
        ...
        for index in list(indexs):
            target_dicoms.append(src_dicoms[index])
        ...
        if is_reversed(array):
            target_dicoms = target_dicoms[::-1]    
        
        return target_dicoms
    ...
    def __getitem__(self, idx):
        ...
        target_dicoms = self.get_target_thickness_dicoms(dicom_dir, target_thick=5)
        
        #### image part ####
        # reverse 적용시: index 0: 몸 아래(골반), index -1: 몸 위(심장)
        image = []
        for dicom in target_dicoms:
...
class TraumaDatasetRaw_seg(Dataset):
    def __init__(self, data_root=Path('data/dataset_origin/train_images/'), 
